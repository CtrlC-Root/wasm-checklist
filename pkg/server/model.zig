...
test "checklist serialization" {
    const sample_checklist_data_list =
        \\[
        \\  {
        \\    "id": 1,
        \\    "title": "Sometime Soon",
        \\    "created_by_user_id": 1,
        \\    "created_on_timestamp": 1745364306
        \\  }
        \\]
    ;

    const checklists = blk: {
        var buffer_stream = std.io.fixedBufferStream(sample_checklist_data_list);
        var json_reader = std.json.reader(std.testing.allocator, buffer_stream.reader());
        defer json_reader.deinit();
        ...
        var checklists = std.ArrayList(Checklist).init(std.testing.allocator);
        errdefer {
            for (checklists.items) |checklist| {
                checklist.deinit(std.testing.allocator);
            }
        ...
        for (data_parsed.value) |checklist_data| {
            ...
            try checklist.init(std.testing.allocator, &checklist_data);
            errdefer checklist.deinit(std.testing.allocator);
    ...
    defer {
        ...
        std.testing.allocator.free(checklists);
    }

    try std.testing.expectEqual(1, checklists.len);
    try std.testing.expectEqual(1, checklists[0].id);
    try std.testing.expectEqualSlices(u8, "Sometime Soon", checklists[0].title);
    try std.testing.expectEqual(1, checklists[0].created_by_user_id);
    try std.testing.expectEqual(1745364306, checklists[0].created_on_timestamp);
}
